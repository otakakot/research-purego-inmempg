...
DROP PROCEDURE IF EXISTS proc_noop();
DROP AGGREGATE IF EXISTS custom_sum(INT);
DROP FUNCTION IF EXISTS int_add_state(INT, INT);
DROP OPERATOR IF EXISTS |+| (TEXT, TEXT);
DROP FUNCTION IF EXISTS text_concat_fn(TEXT, TEXT);
DROP CAST IF EXISTS (posint AS BOOLEAN);
DROP FUNCTION IF EXISTS posint_to_bool(posint);
...
-- =====================
-- [P3] CREATE PROCEDURE / DROP PROCEDURE
-- =====================
CREATE PROCEDURE proc_noop()
LANGUAGE SQL
AS $$
    SELECT 1;
$$;
CALL proc_noop();
DROP PROCEDURE proc_noop();

-- =====================
-- [P4] CREATE AGGREGATE / DROP AGGREGATE
-- =====================
CREATE FUNCTION int_add_state(state INT, val INT) RETURNS INT AS $$
    SELECT state + val;
$$ LANGUAGE SQL;
...
SELECT custom_sum(id) FROM generate_series(1, 5) AS id;  -- expect 15

DROP AGGREGATE custom_sum(INT);
DROP FUNCTION int_add_state(INT, INT);

-- =====================
-- [P4] CREATE OPERATOR / DROP OPERATOR
-- =====================
CREATE FUNCTION text_concat_fn(a TEXT, b TEXT) RETURNS TEXT AS $$
    SELECT a || ' + ' || b;
$$ LANGUAGE SQL;
...
CREATE OPERATOR |+| (
    LEFTARG = TEXT,
    RIGHTARG = TEXT,
    FUNCTION = text_concat_fn
);

SELECT 'hello' |+| 'world';  -- expect 'hello + world'

DROP OPERATOR |+| (TEXT, TEXT);
DROP FUNCTION text_concat_fn(TEXT, TEXT);

-- =====================
-- [P4] CREATE CAST / DROP CAST
-- =====================
-- Note: Custom casts on composite types can trigger recursion with ||.
-- Demonstrate with a domain-based approach instead.
CREATE DOMAIN posint AS INTEGER CHECK (VALUE > 0);

CREATE FUNCTION posint_to_bool(posint) RETURNS BOOLEAN AS $$
    SELECT $1 > 0;
$$ LANGUAGE SQL IMMUTABLE;

CREATE CAST (posint AS BOOLEAN) WITH FUNCTION posint_to_bool(posint);

SELECT CAST(42::posint AS BOOLEAN);  -- expect true

DROP CAST (posint AS BOOLEAN);
DROP FUNCTION posint_to_bool(posint);
DROP DOMAIN posint;
