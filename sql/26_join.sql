...
-- TODO [P3]: LATERAL JOIN with generate_series — expand rows
SELECT e.name, gs.n
FROM employees e,
LATERAL generate_series(1, e.id) AS gs(n)
WHERE e.id <= 3
ORDER BY e.name, gs.n;

-- TODO [P3]: LATERAL JOIN with aggregate — per-row aggregation
SELECT d.name AS department, stats.emp_count, stats.avg_id
FROM departments d
LEFT JOIN LATERAL (
    SELECT COUNT(*) AS emp_count, AVG(e.id) AS avg_id
    FROM employees e
    WHERE e.dept_id = d.id
) stats ON TRUE;

-- TODO [P3]: CROSS JOIN LATERAL — equivalent to comma + LATERAL
