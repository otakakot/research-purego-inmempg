...
-- TODO [P3]: GIN index for tsvector — full text search indexing
ALTER TABLE idx_test ADD COLUMN tsv TSVECTOR;
UPDATE idx_test SET tsv = to_tsvector('english', name);
CREATE INDEX idx_test_tsv_gin ON idx_test USING gin (tsv);

-- TODO [P3]: GiST index for range — range type indexing
ALTER TABLE idx_test ADD COLUMN period TSRANGE;
UPDATE idx_test SET period = tsrange(created::TIMESTAMP, (created + INTERVAL '1 day')::TIMESTAMP);
CREATE INDEX idx_test_period_gist ON idx_test USING gist (period);

-- TODO [P4]: REINDEX CONCURRENTLY — rebuild index without blocking
REINDEX INDEX CONCURRENTLY idx_test_name;

-- TODO [P4]: Index with storage parameter (fillfactor)
CREATE INDEX idx_test_score_ff ON idx_test (score) WITH (fillfactor = 70);

-- TODO [P3]: DROP INDEX CONCURRENTLY
DROP INDEX CONCURRENTLY idx_test_score_conc;
