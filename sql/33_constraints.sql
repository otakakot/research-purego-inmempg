...
INSERT INTO con_exclude (room, period) VALUES
    (2, '[2024-01-01, 2024-01-02)');  -- different room, same time OK
SELECT * FROM con_exclude;
DROP TABLE con_exclude;
DROP EXTENSION IF EXISTS btree_gist;

-- TODO [P3]: NOT VALID constraint + VALIDATE CONSTRAINT
CREATE TABLE con_notvalid (
    id  SERIAL PRIMARY KEY,
    val INT
);
INSERT INTO con_notvalid (val) VALUES (-1), (5), (10);

-- Add constraint as NOT VALID — existing rows are not checked
ALTER TABLE con_notvalid ADD CONSTRAINT chk_positive CHECK (val > 0) NOT VALID;

-- New inserts must satisfy the constraint:
INSERT INTO con_notvalid (val) VALUES (20);
-- Should fail:
-- INSERT INTO con_notvalid (val) VALUES (-5);

-- Validate existing rows (will fail because -1 exists):
-- ALTER TABLE con_notvalid VALIDATE CONSTRAINT chk_positive;

DROP TABLE con_notvalid;

-- TODO [P3]: FOREIGN KEY with ON DELETE SET DEFAULT
CREATE TABLE fk_setdef_parent (
    id   SERIAL PRIMARY KEY,
    name TEXT
);
CREATE TABLE fk_setdef_child (
    id        SERIAL PRIMARY KEY,
    parent_id INT DEFAULT 1 REFERENCES fk_setdef_parent(id) ON DELETE SET DEFAULT
);

INSERT INTO fk_setdef_parent (name) VALUES ('default_parent'), ('other_parent');
INSERT INTO fk_setdef_child (parent_id) VALUES (2);

DELETE FROM fk_setdef_parent WHERE id = 2;
SELECT * FROM fk_setdef_child;  -- expect parent_id = 1 (SET DEFAULT)

DROP TABLE fk_setdef_child;
DROP TABLE fk_setdef_parent;
