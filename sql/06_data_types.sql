...
-- ============================================================================
-- Reference: docs/pg-deep-dive.md §3 - Type system (minimum OID set)
-- The following queries verify pg_typeof()/oid round-trips, text vs binary
-- I/O, and which implicit casts succeed. Implementing the 19 types below
-- covers the vast majority of real-world workloads (pgx default codecs).
-- ============================================================================

-- ----------------------------------------------------------------------------
-- TODO: §3 minimum OID set — pg_typeof / oid for each built-in type — P1
-- Reference: docs/pg-deep-dive.md §3 - pg_type initial data
-- ----------------------------------------------------------------------------
SELECT 'bool'        AS label, pg_typeof(TRUE)::text                  AS pg_type, 'bool'::regtype::oid        AS oid
UNION ALL SELECT 'int2',       pg_typeof(1::int2)::text,                'int2'::regtype::oid
UNION ALL SELECT 'int4',       pg_typeof(1::int4)::text,                'int4'::regtype::oid
UNION ALL SELECT 'int8',       pg_typeof(1::int8)::text,                'int8'::regtype::oid
UNION ALL SELECT 'float4',     pg_typeof(1::float4)::text,              'float4'::regtype::oid
UNION ALL SELECT 'float8',     pg_typeof(1::float8)::text,              'float8'::regtype::oid
UNION ALL SELECT 'numeric',    pg_typeof(1::numeric)::text,             'numeric'::regtype::oid
UNION ALL SELECT 'text',       pg_typeof('x'::text)::text,              'text'::regtype::oid
UNION ALL SELECT 'varchar',    pg_typeof('x'::varchar)::text,           'varchar'::regtype::oid
UNION ALL SELECT 'bpchar',     pg_typeof('x'::char(3))::text,           'bpchar'::regtype::oid
UNION ALL SELECT 'bytea',      pg_typeof('\x00'::bytea)::text,          'bytea'::regtype::oid
UNION ALL SELECT 'date',       pg_typeof(DATE '2024-01-01')::text,      'date'::regtype::oid
UNION ALL SELECT 'time',       pg_typeof(TIME '12:00')::text,           'time'::regtype::oid
UNION ALL SELECT 'timestamp',  pg_typeof(TIMESTAMP '2024-01-01')::text, 'timestamp'::regtype::oid
UNION ALL SELECT 'timestamptz',pg_typeof(TIMESTAMPTZ '2024-01-01Z')::text, 'timestamptz'::regtype::oid
UNION ALL SELECT 'interval',   pg_typeof(INTERVAL '1 day')::text,       'interval'::regtype::oid
UNION ALL SELECT 'uuid',       pg_typeof('00000000-0000-0000-0000-000000000000'::uuid)::text, 'uuid'::regtype::oid
UNION ALL SELECT 'json',       pg_typeof('{}'::json)::text,             'json'::regtype::oid
UNION ALL SELECT 'jsonb',      pg_typeof('{}'::jsonb)::text,            'jsonb'::regtype::oid
ORDER BY oid;

-- ----------------------------------------------------------------------------
-- TODO: §3 text I/O — three equivalent cast spellings — P1
-- Reference: docs/pg-deep-dive.md §3 - typinput / text input function
-- ----------------------------------------------------------------------------
SELECT '42'::int            AS pg_cast,
       int '42'             AS literal_typed,
       CAST('42' AS int)    AS sql_cast;

SELECT '3.14'::numeric         AS num_cast,
       CAST('3.14' AS numeric) AS num_sql_cast,
       numeric '3.14'          AS num_literal;

-- ----------------------------------------------------------------------------
-- TODO: §3 binary-style I/O via bytea round-trip — P2
-- Reference: docs/pg-deep-dive.md §3 - typsend / typreceive
-- ----------------------------------------------------------------------------
SELECT 'hello'::text::bytea          AS as_bytes,
       encode('hello'::bytea, 'hex') AS hex_form,
       decode('68656c6c6f', 'hex')   AS decoded;

-- ----------------------------------------------------------------------------
-- TODO: §3 implicit cast — examples that succeed — P2
-- Reference: docs/pg-deep-dive.md §3 - parse_coerce.c (assignment/implicit)
-- ----------------------------------------------------------------------------
SELECT 1::int + 1::numeric AS int_plus_numeric;        -- int → numeric (implicit)
SELECT 1::int4 + 1::int8   AS int4_plus_int8;          -- int4 → int8 (implicit)
SELECT length('abc')       AS implicit_unknown_to_text; -- 'unknown' → text

-- ----------------------------------------------------------------------------
-- TODO: §3 implicit cast — examples that DO NOT happen (require explicit) — P2
-- Reference: docs/pg-deep-dive.md §3 - typcategory / typispreferred
-- ----------------------------------------------------------------------------
-- These would error without an explicit cast:
--   SELECT '42' || 1;         -- ERROR: operator does not exist: unknown || integer
--   SELECT 1 = '1'::text;     -- ERROR: operator does not exist: integer = text
-- Explicit forms:
SELECT '42'::int || ''            AS need_cast_to_text_first;
SELECT 1 = ('1'::text)::int       AS need_cast_back_to_int;
SELECT 1::text = '1'              AS both_resolved_as_text;
