...
| [推奨アーキテクチャと結論](docs/recommendation.md) | 短期・中長期の推奨事項、関連リンク集 |
...
> 詳細: [推奨アーキテクチャと結論](docs/recommendation.md)

**Go製のインメモリ・インプロセスPostgreSQLを「完全にPure Goで」実現するのは、現時点では非常に困難**である。ただし、用途を限定することで実用的なソリューションは構築可能：

| 用途 | 推奨 |
|------|------|
| テスト（高互換性） | DoltgreSQL インプロセス起動 |
| テスト（基本SQL） | ramsql 利用 |
| テスト（完全互換） | embedded-postgres |

**最も有望な新技術**: [pgplex/pgparser](https://github.com/pgplex/pgparser)（Pure Go PG 17.7パーサー）+ [psql-wire](https://github.com/jeroenrinzema/psql-wire)（Pure Go ワイヤープロトコル）の組み合わせにより、残る課題は**クエリ実行エンジンの実装**に集約される。
