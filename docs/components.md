...
> **注**: PostgreSQLソースコードへの参照は PostgreSQL 19devel（Copyright © 2026）に基づく。
...
PostgreSQL本体のワイヤープロトコル実装は以下のファイルに分散している:

- `src/backend/libpq/pqcomm.c`（2,088行）— 低レベル通信（ソケット接続・切断、バイト送受信）
- `src/backend/libpq/pqformat.c` — メッセージフォーマット（型付きデータのシリアライズ/デシリアライズ）
- `src/backend/tcop/postgres.c`（5,273行）— クエリ処理メインループ（Simple Query / Extended Query の分岐処理）
- プロトコルバージョン: `PG_PROTOCOL(3,0)` から `PG_PROTOCOL(3,2)` まで対応（PG 19devel）
...
### PostgreSQLソースコード参照（パーサー）

- `src/backend/parser/gram.y`（20,059行）— 完全なSQL文法定義（Bison形式）
- `src/include/nodes/parsenodes.h`（4,433行）— パースツリーノード定義（全AST構造体）
- pgparserはPG 17.7（REL_17_STABLE）ベースであり、19develで追加された新しい文法ルールは未対応の可能性がある
...
### PostgreSQLソースコード参照（実行エンジン）

- `src/backend/executor/`（65ファイル）— PostgreSQLエグゼキュータ（Volcano/demand-pullモデルで各ノードが逐次的にタプルを要求・返却）
- `src/backend/optimizer/` — コストベースオプティマイザ（統計情報に基づくプラン選択）
- `src/backend/optimizer/path/costsize.c`（225KB）— コスト推定（シーケンシャルスキャン、インデックススキャン、JOIN等のコスト計算）
- go-mysql-serverのオプティマイザはルールベースで約50ルールであるのに対し、PostgreSQLはコストベースのより洗練されたアプローチを採用している
...
### PostgreSQLソースコード参照（ストレージ）

PostgreSQLのストレージは以下の構造を持つ:

- `src/include/storage/bufpage.h` — 8KBページ構造体の定義（ヘッダ、ItemId配列、タプルデータ）
- `src/backend/access/heap/` — ヒープテーブルアクセス（行の挿入・更新・削除・スキャン）
- `src/backend/access/transam/xact.c`（189KB）— トランザクション管理（MVCC、xmin/xmaxによるタプル可視性制御）
- インメモリ実装では、これらの複雑なページ構造やMVCCメカニズムをGoのデータ構造（map、slice等）に簡略化できる
...
### PostgreSQLソースコード参照（接続モデル）

PostgreSQLは `src/backend/postmaster/postmaster.c` でforkベースの接続モデルを採用しており、クライアント接続ごとに子プロセスを生成する。インメモリ実装ではこのforkモデルをgoroutineに置き換えることで、軽量かつ効率的な並行接続処理が可能になる。
