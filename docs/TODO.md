...
- [ ] P4: `CREATE TABLE ... PARTITION BY` — パーティションテーブル（当面保留）
...
- [ ] P2: `SHOW` — パラメータ値表示
- [ ] P2: `RESET` — パラメータリセット
- [ ] P1: `SET search_path` / `SHOW search_path` — スキーマ解決順の管理
- [ ] P1: `SET client_encoding` / `SHOW client_encoding` — 文字コード設定管理
- [ ] P1: `SET TimeZone` / `SET DateStyle` — 日時解釈と表示形式の制御
- [ ] P2: `SET default_transaction_isolation` — デフォルト分離レベル設定
- [ ] P2: `SET standard_conforming_strings` — 文字列エスケープ挙動制御
- [ ] P3: `LISTEN` / `NOTIFY` — 非同期通知
- [ ] P3: `LOCK` — テーブルロック
- [ ] P4: `VACUUM` — テーブルの保守（インメモリでは原則保留）
- [ ] P3: `ANALYZE` — 統計情報更新
- [ ] P4: `CLUSTER` — テーブルの物理的再編成（インメモリでは原則保留）
- [ ] P4: `REINDEX` — インデックス再構築（インメモリでは原則保留）
- [ ] P3: `DO` — 匿名コードブロック実行
...
- [ ] P1: 暗黙キャスト（代入/比較/式評価） — 型不一致時のPG互換変換
- [ ] P1: 共通型決定（`UNION` / `CASE` / `COALESCE`） — 結果型の決定規則
- [ ] P1: パラメータ型推論（`$1`, `$2`） — 文脈依存型の解決
- [ ] P2: 演算子オーバーロード解決 — 演算子引数型に応じた実装選択
- [ ] P2: 関数オーバーロード解決 — 関数候補からの最適解選択
- [ ] P2: `numeric(p,s)` の精度/スケール規則 — 桁あふれ・丸め挙動
...
### 10.6 プロトコル状態管理・セッション整合性

- [ ] P1: `ReadyForQuery` トランザクション状態 (`I`/`T`/`E`) の整合
- [ ] P1: `ErrorResponse` 後の復帰シーケンス — 次メッセージ受理までの状態遷移
- [ ] P2: Parse/Bind/Describe/Execute のステートメント/ポータルライフサイクル管理
- [ ] P2: `Sync` / `Flush` とパイプライン処理順序の保証
- [ ] P2: `BackendKeyData` を用いた `CancelRequest` 有効期限管理
- [ ] P2: Prepared Statement キャッシュのセッションスコープ管理
- [ ] P2: `ErrorResponse` の SQLSTATE/Severity/Detail/Hint の一貫返却
...
- [ ] P4: `unaccent` — アクセント除去
- [ ] P4: `pgvector` — ベクトル型・近似最近傍検索（特殊用途）

### 11.3 パーティショニング

- [ ] P4: RANGE パーティション — 範囲ベースパーティション（当面保留）
- [ ] P4: LIST パーティション — リストベースパーティション（当面保留）
- [ ] P4: HASH パーティション — ハッシュベースパーティション（当面保留）
- [ ] P4: デフォルトパーティション — 未分類行用パーティション（当面保留）

### 11.4 行レベルセキュリティ (RLS)

- [ ] P4: `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` — RLS有効化（当面保留）
- [ ] P4: `CREATE POLICY` — ポリシー作成（当面保留）
- [ ] P4: `ALTER POLICY` — ポリシー変更（当面保留）
- [ ] P4: `DROP POLICY` — ポリシー削除（当面保留）
...
### 12.1 最低限の互換性テスト (MVPゲート)

- [ ] P1: `pgx` / `database/sql` 接続スモークテスト — Startup〜Simple Queryの疎通
- [ ] P1: 差分テスト（PostgreSQL実機との結果比較） — 基本DDL/DML/SELECT
- [ ] P1: SQLSTATE 互換性テスト — 主要エラーコード（`42P01`, `42703`, `23502`, `42601` 等）
- [ ] P1: RowDescription/DataRow/CommandComplete の整合テスト — 列型/OID/行数

### 12.2 継続的な互換性検証

- [ ] P2: SQLLogicTest ハーネス導入 — 結果正確性の継続検証
- [ ] P2: PostgreSQL 回帰テスト SQL サブセット実行 — 機能回帰の検知
- [ ] P2: ワイヤープロトコル順序テスト — Extended Query のメッセージ列検証
- [ ] P2: `READ COMMITTED` 同時実行シナリオテスト — 可視性/ロック挙動検証
- [ ] P3: パーサー/式評価の Fuzz テスト — 異常系入力耐性
- [ ] P3: 互換性スコア可視化 — Phaseごとの到達率管理
